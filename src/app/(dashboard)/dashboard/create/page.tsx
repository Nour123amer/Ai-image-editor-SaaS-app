...
import { Download, Expand, ImageIcon, Loader2, Minus, RotateCcw, Scissors, Target, Upload, X } from 'lucide-react';
...
export default function createPage() {
    ...
    const initializeData = async () => {
      try {
        ...
        if (projectResult.success && projectResult.projects) {
          setUserProjects(projectResult.projects)
        }
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (projectResult.success) {
          ...
          console.log("failed to save project to db");
        }
  ...
  //  get live preview transformation
  const getLivePreviewTransformation = () => {
    return [...transformations]
  }

  // check if transformation exits
  ...
  const removeTransformation = (type: string) => {
    ...
    toast.success(`${type.charAt(0).toUpperCase() + type.slice(1)} transformation removed!`)
  }
  ...
  const removeBackground = async () => {
    ...
    // check if remove background already applied
    if (hasTransformation("background")) {
      toast.error("Background removal already applied !");
      return;
    }
  ...
  const upscaleImage = async () => {
    ...
    if (hasTransformation("upscaling")) {
      toast.error("upscale already applied!");
      return;
    }
  ...
  const objectCrop = async () => {
    if (!upLoadedImage) return;
    //check if crop already applied
    if (hasTransformation("objectCrop")) {
      toast.error("crop already applied!");
      return;
    }


    if (!objectInput.trim()) {
      toast.error("please enter object");
      return;
    }
    setIsProcessing(true);
  ...
  const clearTransformations = () => {
    setTransformations([]);
    toast.success("All transformation removd!");
  }
  ...
  return (
    <>
      ...
      <SignedIn>
        ...
        <div className="grid grid-cols-1 gap-2 px-4 sm:gap-4 lg:grid-cols-3">
                ...
                <div className="space-y-2">
                  <div className="grid gap-2 sm:grid-cols-1">
                    ...
                    <div className='group relative'>
                      ...
                      {hasTransformation("upscale") && (
                        <Button onClick={() => removeTransformation("upscale")}
                          disabled={isProcessing}
                          variant="destructive"
                          size="sm"
                          className="absolute top-0.5 right-0.5 h-4 w-4 rounded-full p-0 opacity-0 transition-opacity group-hover:opacity-100"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className='flex gap-1'>
                        ...
                        >
                          ...
                          <span className='text-xs'>
                            {isProcessing ?
                              "Processing..." : hasTransformation("objectCrop") ? "applied" : "Apply"}
