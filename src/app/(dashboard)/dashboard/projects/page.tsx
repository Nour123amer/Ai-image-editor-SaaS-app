      ...
      <div className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <Tabs defaultValue={direction} onValueChange={(v) => setDirection(v)}>
          <TabsList className="grid w-full grid-cols-2 h-10 shadow-sm border">
            <TabsTrigger value="grid" onClick={() => setDirection("vertical")}>
              <LayoutGrid className="w-4 h-4 mr-2" /> Grid
            </TabsTrigger>
            <TabsTrigger value="list" onClick={() => setDirection("horizontel")}>
        ...
        {userProjects.length === 0 ? (
          ...
          <div className={`grid gap-6 ${
            ...
            {userProjects.map((project: any) => (
              <div
                key={project.id}
                className={`group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all duration-300 overflow-hidden flex ${
                  direction === "horizontel" ? "flex-row h-40" : "flex-col"
                }`}
              >
                {/* Thumbnail Container */}
                <div className={`relative overflow-hidden bg-slate-100 ${
                  direction === "horizontel" ? "w-64 shrink-0" : "aspect-[16/10]"
                }`}>
                  <img
